...
<x-app-layout>
    ...
    <div class="min-h-screen bg-admin p-6 sm:p-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            <div>
                ...
                <p class="text-slate-500 font-medium text-sm mt-1">Plotting koordinat jawaban wajib untuk Misi Simulasi.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-8 space-y-4">
                ...
                <div class="flex items-center space-x-3 text-slate-400 px-2">
                    ...
                    <span class="text-xs font-medium">Klik pada gambar untuk menentukan jawaban target pengerjaan siswa.</span>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="admin-card p-8 shadow-sm">
                    ...
                    <div id="sortable-list" class="space-y-3 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <div class="p-4 bg-white border border-slate-100 rounded-2xl flex justify-between items-center group cursor-move hover:border-emerald-300 transition-all shadow-sm" data-id="{{ $hs->id }}">
                            ...
                            <form action="{{ route('admin.missions.destroy-hotspot', $hs->id) }}" method="POST" class="m-0 flex items-center">
                                @csrf @method('DELETE')
                                <button type="submit" onclick="return confirm('Hapus target jawaban ini?')" class="w-9 h-9 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                        ...
                        <div class="text-center py-10 text-slate-300">
                            <p class="text-[10px] font-bold uppercase tracking-widest">Belum ada titik target</p>
                        </div>
                        @endforelse
                    </div>
                </div>
        ...
        <footer class="mt-20 py-8 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between opacity-50 px-2">
            <div class="text-left leading-tight">
                ...
                <p class="text-[9px] font-medium text-slate-400 uppercase mt-1">Virtual Lab Excel Mission Logic</p>
            </div>
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Pure Plotting Engine</span>
        </footer>
    </div>
    ...
    <script>
        ...
        function setPoint(event) {
            ...
            btn.innerHTML = `Simpan Target #${ {{ count($step->hotspots ?? []) }} + 1 }`;
        }
    </script>
</x-app-layout>