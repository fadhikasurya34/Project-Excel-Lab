...
<x-app-layout>
    ...
    <div class="min-h-screen bg-admin p-4 sm:p-10"> {{-- Padding responsif --}}
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 sm:mb-12 gap-6">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 text-header tracking-tight">Manajemen <span class="text-emerald-600">Topik Misi</span></h1>
                <p class="text-slate-500 font-medium text-xs sm:text-sm mt-1">Kelola tantangan laboratorium berdasarkan kategori topik excel.</p>
            </div>
            <a href="{{ route('admin.missions.create') }}" class="btn-emerald inline-flex items-center justify-center px-6 sm:px-8 py-3.5 sm:py-4 w-full md:w-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-10">
            ...
            <div class="admin-card p-5 sm:p-6 flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl sm:text-3xl font-black text-slate-900">{{ $missions->count() }}</h3>
                </div>
                <div class="w-10 h-10 sm:w-12 sm:h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center">
            ...
            <div class="admin-card p-5 sm:p-6 flex items-center justify-between sm:col-span-2 md:col-span-1">
                <div>
                    ...
                    <h3 class="text-2xl sm:text-3xl font-black text-slate-900">{{ number_format($missions->sum('max_score')) }}</h3>
                </div>
                <div class="w-10 h-10 sm:w-12 sm:h-12 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse block md:table">
                ...
                <tbody class="divide-y divide-slate-100 block md:table-row-group">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group block md:table-row p-5 md:p-0 mb-4 md:mb-0 bg-white md:bg-transparent rounded-2xl md:rounded-none border border-slate-100 md:border-none shadow-sm md:shadow-none">
                        ...
                        {{-- Komposisi Tipe (Hidden label di desktop, visible di portrait) --}}
                        <td class="block md:table-cell py-3 md:py-6 md:px-8 md:text-center">
                            <div class="flex md:contents items-center justify-between">
                                <span class="md:hidden text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tipe Misi</span>
                        ...
                        <td class="block md:table-cell py-4 md:py-6 md:px-8 md:text-right border-t border-slate-50 md:border-none mt-4 md:mt-0">
                            <div class="flex items-center justify-between md:justify-end md:space-x-3">
                                ...
                                <div class="flex items-center space-x-2 sm:space-x-3">
                                    <a href="{{ route('admin.missions.topic', $cat->category) }}" class="inline-flex px-4 py-2 sm:px-5 sm:py-2.5 bg-emerald-600 text-white text-[9px] sm:text-[10px] font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-md shadow-emerald-50 uppercase tracking-widest">
                                        Kelola
                                    </a>
                                    <button onclick="openEditModal('{{ addslashes($cat->category) }}', '{{ addslashes($cat->description) }}')" class="p-2 text-slate-400 hover:text-emerald-600 transition-colors">
                                    ...
                                    <form action="{{ route('admin.missions.destroy-topic', $cat->category) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" onclick="return confirm('Hapus seluruh topik?')" class="p-2 text-slate-400 hover:text-red-600 transition-colors">
    ...
    <div id="editTopicModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl z-50 w-full max-w-md overflow-hidden transform transition-all border border-slate-100 p-6 sm:p-10">
