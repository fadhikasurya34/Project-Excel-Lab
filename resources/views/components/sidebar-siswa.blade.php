{{-- 
    DOCUMENTATION: SIDEBAR SISWA (REVISI SILVER STANDARD)
--}}
...
<aside x-show="sidebarOpen" x-cloak
    ...
    <style>
        ...
        /* Khusus Logout (Tetap Merah untuk Alert) */
        .menu-logout:hover { 
            border-color: #fecaca; 
        ...
        .dark .menu-logout:hover { background: rgba(239, 68, 68, 0.1) !important; }
    ...
    <div class="flex-1 overflow-y-auto scrollbar-hide p-5 md:p-6">
        ...
        {{-- Section: Close & Ticket --}}
        <div class="flex items-center justify-between mb-8">
            <button @click="sidebarOpen = false" 
                    class="w-10 h-10 flex items-center justify-center rounded-xl btn-pegas bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 text-slate-400 hover:text-red-500 transition-all">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3.5">
                    <path d="M15 19l-7-7 7-7"/>
                </svg>
            </button>

            <div class="flex items-center space-x-2.5 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm">
                <img src="{{ asset('images/tiket.png') }}" class="w-5 h-5 object-contain">
                <div class="flex flex-col leading-none">
                    <span class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Retry Available</span>
                    <span class="text-[11px] font-black text-slate-700 dark:text-slate-300 font-game mt-0.5">{{ $remainingTickets }} LEFT</span>
                </div>
            </div>
        </div>
        ...
        <div class="text-center mb-10">
            <div class="relative w-24 h-24 mx-auto mb-4 profile-float">
                <div class="absolute inset-0 rounded-[2.2rem] border-[3px] border-dashed opacity-30 profile-ring" style="border-color: #{{ $userColor }};"></div>
                <div class="absolute inset-1.5 rounded-[2rem] flex items-center justify-center shadow-xl overflow-hidden border-4 border-white dark:border-slate-800" style="background: #{{ $userColor }};">
                    <img src="https://api.dicebear.com/9.x/bottts/svg?seed={{ $user->avatar ?? 'Felix' }}&backgroundColor=transparent" class="w-full h-full object-cover transform scale-110 pt-2 transition-transform duration-300">
                </div>
            </div>
            <h2 class="text-[9px] font-black tracking-[0.2em] uppercase mb-1 opacity-60 dark:text-slate-400">Student Profile</h2>
            <p class="text-xl font-black text-slate-800 dark:text-white leading-tight capitalize truncate">{{ $user->name }}</p>
        </div>

        {{-- MENU CARDS (STANDARD SILVER) --}}
        <div class="flex flex-col space-y-4">
            
            {{-- Akumulasi XP --}}
            <div class="sb-card menu-silver group">
                <div class="flex items-center space-x-3.5">
                    ...
                    <div class="text-left">
                        <p class="text-[9px] font-black text-slate-400 uppercase mb-0.5">Power Points</p>
            ...
            <div class="sb-card menu-silver group">
                <div class="flex items-center space-x-3.5">
                    ...
                    <div class="text-left">
                        <p class="text-[9px] font-black text-slate-400 uppercase mb-0.5">Title Rank</p>
            ...
            <div class="sb-card menu-silver group">
                <div class="flex items-center space-x-3.5 flex-1 min-w-0">
                    ...
                    <div class="text-left truncate">
                        <p class="text-[9px] font-black text-slate-400 uppercase mb-0.5">Squad Lab</p>
                        <p class="text-[11px] font-bold text-slate-700 dark:text-slate-200 truncate">{{ $firstClass ? $firstClass->name : 'No Squad' }}</p>
                    </div>
                </div>
            </div>

            <hr class="border-dashed border-slate-200 dark:border-slate-800 my-2">

            {{-- LOGOUT (Ikut di dalam scroll list) --}}
            <form method="POST" action="{{ route('logout') }}" class="pb-10">
                @csrf
                <button type="submit" class="sb-card menu-logout w-full flex items-center justify-center space-x-3 group active:scale-95">
